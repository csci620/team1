...
@Component({
  selector: 'app-tourpanel',
  templateUrl: './tourpanel.component.html',
  styleUrls: ['./tourpanel.component.css']
})
...
outDate = new Date();
   ...
   hotel=  {
    ...
    hotelNameOpt: '0',
   checkIn: this.cDate.toISOString().substring(0, 10),
  checkOut: this.outDate.toISOString().substring(0, 10),
   duration: '5',
members: '2'
  ...
  constructor(private hotelService: HotelsService, private http: HttpClient,
    ...
    private ngZone: NgZone, public auth: AuthService,  private router: Router, private fb : FormBuilder) {
  ...
  updateHotel(ind, data): void {
   ...
   submitData(): void {
    ...
    const newHotel = {
      ...
      hotelName: this.hotel.hotelNameOpt,
      checkIn: this.hotel.checkIn,
      checkOut: this.hotel.checkOut,
      duration: this.hotel.duration,
      members: this.hotel.members
      
      
    };

    
    console.log(" chekin => " + newHotel.checkIn)
      ...
      //  this.hotels.push(response);
        ...
        .subscribe(
          data => {
            ...
            this.hotels =    data;
           this.message = "Hotel Reservation confirmed for " + newHotel.hotelName;
